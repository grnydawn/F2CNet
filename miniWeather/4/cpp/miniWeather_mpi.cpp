/*
 * Llama and Claude consistenly generate superier results than OpenAI and Gemini
 * - follow user's direction 
 * - using pnetcdf correctly between fortran and c versions
...
public:
    ...
    void reductions(real &mass, real &te) {
        ...
        // Meta AI
        //double buffer[2] = {mass, te};
        //MPI_Allreduce(buffer, glob.data(), 2, MPI_REAL_TYPE, MPI_SUM, MPI_COMM_WORLD);
        // Claude
        std::array<double, 2> temp = {mass, te};
        MPI_Allreduce(temp.data(), glob.data(), 2, MPI_REAL_TYPE, MPI_SUM, MPI_COMM_WORLD);
        // Gemini
        //MPI_Allreduce(&mass, &glob[0], 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
        //MPI_Allreduce(&te, &glob[1], 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
    ...
    void output(const std::vector<std::vector<std::vector<real>>>& state, real etime) {
        ...
        if (etime == 0) {
            ...
            int dims[3] = {t_dimid, z_dimid, x_dimid};
            #ifdef SINGLE_PREC
...
private:
    ...
    std::vector<std::vector<std::vector<real>>> state_tmp, flux, tend;
...
int main(int argc, char** argv) {
    ...
    while (simulation.etime < sim_time) {
        // If the time step leads to exceeding the simulation time, shorten it for the last step
        if (simulation.etime + simulation.dt > sim_time) simulation.dt = sim_time - simulation.etime;
        ...
        // Inform the user
#ifndef NO_INFORM
        if (simulation.mainproc) std::cout << "Elapsed Time: " << simulation.etime << " / " << sim_time << std::endl;
#endif
        ...
        if (simulation.output_counter >= output_freq) {
            simulation.output_counter -= output_freq;
            simulation.output(simulation.state, simulation.etime);
        }
    }
